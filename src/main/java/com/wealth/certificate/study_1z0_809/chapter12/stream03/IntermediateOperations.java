...
public class IntermediateOperations {
	...
	public static void main(String[] args) {
		...
		*/
		...
		IntStream numberStream = IntStream.range(1, 21);
		System.out.print("skip-skip-unordered-toList: ");
		numberStream.parallel().filter(n -> n != 0)
				.skip(1)
				.skip(1)
				.unordered()
				.forEach(System.out::print);
		
		numberStream = IntStream.range(1, 21);
		System.out.println("skip-unordered-skip-toList: ");
		numberStream.parallel().filter(x -> x > 0)
	            .skip(1)
	            .unordered()
	            .skip(1)
	            .forEach(System.out::print);
